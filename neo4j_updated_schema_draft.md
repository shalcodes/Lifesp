...
### 1.3 Interactive Schema Explorer

The explorer should be generated from the structured schema documentation from Section 2 onwards, including design principles, schema backbone paths, node definitions, relationship definitions, domain-specific modelling rules, node property catalogues, relationship attribute catalogues, implementation guidance, and future extensions.

The explorer should not only display graph nodes and relationships. It should also expose the explanatory schema content that helps readers understand why each modelling choice exists.

#### Explorer content coverage

The interactive explorer should index and expose the following schema sections and subsections.

| Section | Content to expose in the explorer |
|---|---|
| `2. Design Principles` | High-level modelling rules that guide how the graph should be interpreted and extended. |
| `2.1 Naming Conventions` | Node label, relationship type, property, identifier, and provenance naming rules. |
| `2.2 Relationship Property Modelling Philosophy` | Rules for deciding whether information belongs on a node property, relationship property, observation node, intermediate node, or future evidence/assertion node. |
| `2.3 Stable Entities vs Observation Nodes` | Explanation of why measurements are represented as observation nodes rather than direct entity properties. |
| `2.4 Multi-labelling Strategy` | Rules for multi-labelled nodes such as `Assay:SingleCellAssay` and `Intervention:DrugIntervention`. |
| `2.5 Provenance, Config, and Anti-Hardcoding` | Provenance fields, configuration keys, source-field tracking, and anti-hardcoding principles. |
| `2.6 External Matrix and File Storage Strategy` | Rules for keeping dense molecular matrices external and representing them through `ProcessedDataOutput`. |
| `2.7 Confidence Score Usage Rules` | Guidance on when confidence scores may or may not be populated. |
| `3. Core Schema Backbone` | High-level schema paths and backbone traversal routes. |
| `3.1 Core Study-Sample-Assay Path` | Core provenance path from publication/study to sample, assay, processing run, and processed output. |
| `3.2 Clinical Context Path` | Subject-to-disease clinical context path. |
| `3.3 Molecular Observation Backbone` | Molecular observation layer, if populated in the Markdown. |
| `3.4 Single-Cell Extension Backbone` | Single-cell assay, cell-set, cell-type, cell-state, output, feature, and gene mapping paths. |
| `3.5 Intervention Backbone` | Subject/intervention, experimental condition/intervention, intervention/agent, and intervention/outcome paths. |
| `3.6 Repository Hosting Backbone` | Repository-hosted dataset path separated from the biological root. |
| `3.7 Static High-Level Schema Overview` | Static Mermaid overview only; not the full explorer. |
| `4. Core Node Definitions` | Node labels, descriptions, and key identifiers. |
| `5. Core Relationship Definitions` | Active relationship patterns, relationship types, source nodes, target nodes, and purposes. |
| `5.1 Study and Sample Flow` | Study, subject, sample, organism, tissue, organ, stage, and pairing relationships. |
| `5.2 Experimental and Clinical Context` | Disease, phenotype, phenotype term, and condition relationships. |
| `5.3 Assay and Provenance` | Assay, processing run, processed output, and observation-output relationships. |
| `5.4 Molecular Observation Relationships` | Methylation, expression, and variant observation relationships. |
| `5.5 Genomic and Regulatory Reference Relationships` | CpG, gene, enhancer, variant, and genomic region relationships. |
| `5.6 Methylation Rule and Classification Relationships` | Methylation rule classification relationship. |
| `5.7 Text, Evidence, and Retrieval Relationships` | Publication-level evidence relationships. |
| `5.8 Single-Cell Dataset Relationships` | Repository, single-cell dataset, cell-set, cell-type, cell-state, feature, and gene mapping relationships. |
| `5.9 Intervention Relationships` | Intervention, drug agent, and outcome phenotype relationships. |
| `6. Domain-Specific Extensions` | Domain-specific modelling explanations that clarify how each schema area should be used. |
| `6.1 Clinical and Phenotype Modelling` | Clinical and phenotype-specific modelling notes, if populated. |
| `6.2 Transcriptomic-Specific Design` | Expression observation and transcriptomics-specific modelling rules. |
| `6.3 Epigenome and Methylation-Specific Design` | Methylation beta values, derived methylation status, rules, and evidence support. |
| `6.4 Genomic-Specific Design` | Stable variant versus sample-specific variant observation modelling. |
| `6.5 Variant-Gene Relationship Semantics` | Difference between `AFFECTS_GENE` and `IMPACTS_GENE`. |
| `6.6 Single-Cell Dataset Design` | Single-cell repository, study root, AnnData, CellSet, CellType/CellState, and source-field modelling. |
| `6.6.1 Public Single-Cell Repository Sources` | Public single-cell repositories supported by the architecture. |
| `6.6.2 Single-cell Conceptual Root` | Why `Study`, not `Repository`, is the conceptual biological root. |
| `6.6.3 AnnData / h5ad Explanation` | How AnnData components map to LifeSphere entities. |
| `6.6.4 CellSet-only Representation` | Why LifeSphere uses `CellSet` rather than individual `Cell` nodes. |
| `6.6.5 CellType vs CellState` | Distinction between stable cell identity and context-specific cell state. |
| `6.6.6 sourceField and sourceValue` | Source-field/source-value tracking for reproducibility. |
| `6.7 Intervention-Specific Multi-labelling Rules` | Rules for intervention subtype labels. |
| `6.8 Paired Sample Reference Role` | Meaning and usage of `referenceRole` on `PAIRED_WITH`. |
| `7. Node Property Catalogue` | Node properties, keys, data types, descriptions, examples, and source/origin fields. |
| `7.1 Consensus/Core Node Properties` | Core node property definitions. |
| `7.2 Genomic Node Properties` | Genomic node property definitions. |
| `7.3 Transcriptomic Node Properties` | Expression observation property definitions. |
| `7.4 Epigenome and Methylation Node Properties` | CpG and methylation property definitions. |
| `7.5 Single-Cell Node Properties` | Repository, single-cell dataset, library, CellSet, CellType, CellState, and Feature properties. |
| `7.6 Intervention Node Properties` | Base and subtype-specific intervention properties, plus ChemicalEntity properties. |
| `8. Relationship Attribute Catalogue` | Relationship properties, keys, data types, descriptions, and examples. |
| `8.1 Study and Sample Flow Relationship Attributes` | Attributes for study, sample, tissue, organism, pairing, and developmental-stage relationships. |
| `8.2 Experimental and Clinical Context Relationship Attributes` | Attributes for disease, phenotype, and condition relationships. |
| `8.3 Assay and Provenance Relationship Attributes` | Attributes for assay, processing, output, and generated-observation relationships. |
| `8.4 Molecular Observation Relationship Attributes` | Attributes for molecular observation relationships. |
| `8.5 Genomic and Regulatory Reference Relationship Attributes` | Attributes for regulatory, variant-gene, CpG-gene, and genomic-region relationships. |
| `8.6 Methylation Rule and Classification Relationship Attributes` | Attributes for `CLASSIFIED_USING`. |
| `8.7 Text, Evidence, and Retrieval Relationship Attributes` | Attributes for publication-level `EVIDENCES` relationships. |
| `8.8 Single-Cell Dataset Relationship Attributes` | Attributes for repository, CellSet, annotation, feature, and gene-mapping relationships. |
| `8.9 Intervention Relationship Attributes` | Attributes for intervention, agent, and outcome phenotype relationships. |
| `9. Implementation Guidance and Example Queries` | Implementation rules, indexing guidance, agent-retrieval rules, and example queries. |
| `9.1 Constraints and Index Recommendations` | Constraints and indexes for stable identifiers and lookup fields. |
| `9.2 Agent Retrieval and Text-to-Cypher Guidance` | Agent rules for schema-aware retrieval. |
| `9.3 Example Cypher Queries` | Example query patterns that the explorer may link to relevant node/relationship paths. |
| `9.4 Schema Visualisation Assets` | Repository implementation and maintenance guidance for visualisation assets. |
| `10. Future Schema Extensions` | Future-only schema components, hidden by default in the explorer. |
| `10. RAG, Document Chunking, and Chunk-Level Evidence Grounding` | Future RAG/document chunking model. |
| `10. DocumentChunk` | Future document-chunk node definition. |
| `10. Future chunk-level properties` | Future chunk-level provenance properties. |
| `10. Future chunk-level relationship patterns` | Future chunk-level evidence relationships. |
| `10. Future chunk-level relationship attributes` | Future relationship attributes for document chunking. |

To keep the explorer readable, the default view should be moderated rather than showing the entire schema at once.

The default view should show:

- the major schema sections from Section 2 onwards as a navigable documentation tree;
- the active core graph backbone;
- active node labels and relationship types;
- active node and relationship property summaries.

Detailed property tables, relationship attributes, domain explanations, and future-only extensions should be available through search, filters, collapsible panels, or click-to-inspect views rather than displayed all at once.

Section 2 is part of the explorer source because it defines the modelling rules required to interpret the rest of the schema correctly.

For example, a node such as `Assay:SingleCellAssay` should link back to Section 2.4 Multi-labelling Strategy, and a relationship attribute such as `referenceRole` should link back to Section 2.2 Relationship Property Modelling Philosophy and Section 6.8 Paired Sample Reference Role.

The explorer should support:

- navigation by schema section and subsection from Section 2 onwards;
- search by node label;
- search by relationship type;
- search by property name;
- search by source node and target node;
- search by schema concept, for example `multi-labelling`, `observation node`, `CellSet`, `confidenceScore`, or `referenceRole`;
- layer filters for design principles, core backbone, clinical, assay/provenance, molecular observation, genomic/regulatory, single-cell, intervention, evidence, implementation guidance, and future extensions;
- click-to-inspect node details;
- click-to-inspect relationship details;
- click-to-inspect property details;
- cross-links from nodes, relationships, and properties back to the relevant Markdown section;
- active/future status filtering;
- default exclusion of future-only RAG/DocumentChunk components.

### 1.4 Source Files and Schema Authority
